...
class sp_cdm_media_viewfile{
	...
	$r = $wpdb->get_results("SELECT *  FROM ".$wpdb->prefix."sp_cu   where id= '".$wpdb->escape($fid)."' AND date = '".$wpdb->escape($file_date)."'  AND file = '".$wpdb->escape($file_name)."'", ARRAY_A);
  ...
  Your browser does not support the video tag.
	...
	function tab_name($html,$r){
		...
		if($style == 'tab' && (in_array($ext,array('mp4','ogg','webm','mp3')) or $r[0]['file'] == 'embed')){
		 $html .= '<li><a href="#cdm-media">'.get_option('sp_cdm_media_word','Video').'</a></li>';
		}

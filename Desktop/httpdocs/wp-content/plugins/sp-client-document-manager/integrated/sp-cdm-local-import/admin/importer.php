...
add_action('admin_init', array('sp_cdm_local_import_admin_integrated', 'session'));
...
class sp_cdm_local_import_admin_integrated{
	...
	function ajax_check_files(){
		...
		echo sp_cdm_local_import_admin_integrated::count_local_files($_POST['path']);
		echo '</div>';
...
ini_set('upload_max_filesize', '1024M');
	  ...
	  sp_cdm_local_import_admin_integrated::copy_local_files($_POST['path'],$_POST['uid']);
...
function add_file($file,$pid,$uid){
	...
	$filename = sp_cdm_local_import_admin_integrated::random_hash(sanitize_file_name(basename($file)));
...
/*
   #print_r($_SESSION);
    foreach (scandir($dir) as $node) {
        ...
        if (!is_dir($dir . '/' . $node)) {
				...
				sp_cdm_local_import_admin_integrated::add_file($dir . '/' .$node,$uid);
			
		
        } else {
        $insert_folder = sp_cdm_local_import_admin_integrated::add_folder($node,$uid);
		
         $contents[$node] =  sp_cdm_local_import_admin_integrated::copy_local_files($dir . '/' . $node,$uid);
	...
	*/
	...
	if($_SESSION['start'] ==1){
		...
		$structure = sp_cdm_local_import_admin_integrated::find_folder_structure($dir,$pid);
			ksort($structure);
	...
	exit;
			...
			echo '<div style="margin:10px;padding:10px;background-color:#EFEFEF;border:1px solid #CCC;border-radius:10px;">';
		echo '<pre>';
	 sp_cdm_local_import_admin_integrated::process($pid,$_POST['uid']);
	 echo '</pre>';
	 		echo '</div>';
	}

	#sp_cdm_local_import_admin_integrated::copy_local_files_process($contents,$uid,$destination);
...
$folder_count= 0;
	...
	$path_dir =   array_values(array_filter(explode("/", $_POST['path'])));
	...
	$starting_path = $path_dir[$total];
		...
		#echo '<h2>Current FOlder:'.$current_folder.' ||'. $starting_path.'	';
			...
			if($current_folder != $starting_path){
			$set_pid = sp_cdm_local_import_admin_integrated::add_folder($current_folder ,$parent,$uid);
			...
			foreach($files as $file_key=>$file){
				...
				if( is_numeric($file_key ) && !is_dir($path.'/'.$file)){
					...
					sp_cdm_local_import_admin_integrated::add_file($path.'/'.$file,$set_pid,$uid);
					$file_count++;
...
$folder_count= 0;
	...
	if($loop_total >= $total){
		...
		if($path ==$_POST['path']){
			...
			sp_cdm_local_import_admin_integrated::add_file($path.'/'.$file,$pid,$uid);
			$file_count++;
			}
			}
				...
				$parent_folder = $loop_path_dir[$loop_parent];
					...
					#echo 'parent:'.$parent_path.'_'.$parent_folder.'';
						...
						$pid = sp_cdm_local_import_admin_integrated::add_folder($current_folder ,$parent,$uid);
						$folder_count++;
...
function plotTreeCreate_folders($arr, $indent=0, $mother_run=true,$pid = 0){
    ...
    foreach ($arr as $k=>$v){
		...
		if(is_array($v)){
			$pid = sp_cdm_local_import_admin_integrated::add_folder($k,$pid);
			 sp_cdm_local_import_admin_integrated::plotTree($v, ($indent+1), false,$pid);
		}
    ...
    foreach ($arr as $k=>$v){
        ...
        $show_val = (is_array($v) ? $v["__base_val"] : $v);
	  	...
	  	if(!is_array($v)){
			...
			sp_cdm_local_import_admin_integrated::add_file($v,$pid);
		}
...
function get_folder_structure($dir){
	...
	return sp_cdm_local_import_admin_integrated::explodeTree($files, $delimiter = '/',true);
...
function explodeTree($array, $delimiter = '_', $baseval = false)
{
	...
	add_submenu_page('sp-client-document-manager', __("Local Import", "sp-cdm"), __("Local Import", "sp-cdm"), 'sp_cdm', 'sp-client-document-manager-local-import', array(
        'sp_cdm_local_import_admin_integrated',
        'view'
    ));

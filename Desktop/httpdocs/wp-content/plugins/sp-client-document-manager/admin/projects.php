...
<a href="admin.php?page=sp-client-document-manager-projects&function=edit&id=' . $r[$i]['id'] . '" >' . __("Modify", "sp-cdm") . '</a></td>
        ...
        function view()
        {
            ...
            if ($_POST['save-project'] != "") {
               ...
               if($_POST['parent'] != ''){
			    ...
			    if ($_POST['id'] != "") {
                    ...
                    $wpdb->update("" . $wpdb->prefix . "sp_cu_project", $insert, $where);
                  #flush cache
				  $wpdb->query( "DELETE FROM ".$wpdb->options." WHERE 'option_name' LIKE '_transient_cdm_folder_permissions_".$where['id']."_%'
			...
			$spcdm_ajax->remove_cat(sanitize_text_field($_POST['id']));

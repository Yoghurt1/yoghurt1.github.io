...
class cdm_recycle_bin{
	...
	<strong>' . __("File", "sp-cdm") . ': </strong> ' . stripslashes($r[0]['file']) . '	
		...
		<strong>' . __("File Owner", "sp-cdm") . ': </strong> '.$owner->display_name .'
	...
	<strong>' . __("Folder Parent", "sp-cdm") . ': </strong> ' . stripslashes( sp_cdm_get_project_name($r[0]['parent'])) . '	
		...
		function install(){
		$alters = array();	
		if(get_option('cdm_recycle_bin_install') == '' or get_option('cdm_recycle_bin_install') < $this->db_ver){
				global $wpdb;
		...
		if(get_transient('sp_cdm_recycle_bin_run') == false){

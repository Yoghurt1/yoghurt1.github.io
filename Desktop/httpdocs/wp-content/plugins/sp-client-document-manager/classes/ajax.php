...
class spdm_ajax
{
    ...
    function view_file($file_id = false)
    {
		...
		if(cdm_folder_permissions($r[0]['pid']) or $current_user->ID == $r[0]['uid'] or is_admin())
...
dashicons-arrow-down-alt cdm-dashicons"></span> ' . __("Download File", "sp-cdm") . '</a> ';
    ...
    $html .= '<em>' . date('F jS Y h:i A', strtotime($r[0]['date'])) . ' &bull; File ID: #' . $r[0]['id'] . ' </em>';
	}
	...
	<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>' . __("Are you sure you would like to delete this category? Doing so will remove all files related to this category.", "sp-cdm") . '</p>
	 ...
	 echo '
        ...
        if ($_GET['sort'] == '') {
			...
			$search_file = apply_filters("sp_cdm_file_search_query", $search_file, $_GET['pid']);
           	$query = "SELECT *  FROM " . $wpdb->prefix . "sp_cu   where (uid = '" . $_GET['uid'] . "' ".$r_projects_groups_addon_search.")  	 " . $search_file . " and parent = 0 order by " . $sort . " ";
    ...
    function email_vendor()
    {
		...
		$post['vendor-message'] = sanitize_text_field($_POST['vendor-message']);
			...
			//$headers = apply_filters('spcdm_admin_email_headers',$headers,$post, $uid);
		     ...
		     wp_mail(sanitize_text_field( $_POST['vendor']), strip_tags(stripslashes($subject)),stripslashes( $message),apply_filters('sp_cdm/mail/headers',$headers, wp_get_current_user(),$to,$subject,$message), $attachments);
